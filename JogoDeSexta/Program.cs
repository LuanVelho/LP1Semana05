...
namespace JogoDeSexta
{
    public class Level
    {
        public Level(int NofRooms, toughtness)
        {
            SetEnemyInRoom(int Indice, string inimigo)
            {
                return Indice, inimigo;
            }

            private GetLevel2()
            {
                return Level2.toughtness;
            }

            private PrintEnemy()
            {
                foreach(Inimigo in Enemy)
                {
                    Console.WriteLine($"{EnemyInRoom}:{Inimigo}");
                }
            }
